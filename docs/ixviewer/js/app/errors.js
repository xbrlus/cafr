...
/*
 * Below we are using the prototype functionality of JavaScript to extend the
 * base Error function Common practice, this will 'clean up' the console
 * error(s) a user may have
 */
function CustomCORSError(message, hostName, fileHostName) {
    this.name = 'CORS Error';
    this.message = (message || 'The Host name ('
	    + hostName
	    + ') cannot be different from the file you are trying to retrieve ('
	    + fileHostName + ')');
}
...
function CustomNotFoundError(message, file) {
    ...
    this.message = (message || 'The file you are trying to retrieve (' + file
	    + ') could not be located.')
}
CustomNotFoundError.prototype = Error.prototype;

/*
 * Below we use the onerror function to output our above errors properly AKA
 * they are no longer 'Uncaught'
 */
